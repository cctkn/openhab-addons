...
export default class TypeDefinition {
  public static parse(data: any): TypeDefinition {
    ...
    for (const chl of data.channels) {
      ...
      const mappedChannelId: string =
        chl.mappedChannelId === chl.channelId ? undefined : chl.mappedChannelId;
      ...
      channels.push(
        new Channel(channelId, mappedChannelId, channelType, properties, state)
      );
    }
  ...
  constructor(
    ...
    this.channelGroups = (channelGroups === undefined
      ? []
      : channelGroups
    ).sort((a, b) => a.name.localeCompare(b.name));
    this.channels = (channels === undefined ? [] : channels).sort((a, b) =>
      a.channelWithGroup.localeCompare(b.channelWithGroup)
    );
  }
